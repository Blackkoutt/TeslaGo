...
namespace TeslaGoAPI.DB.Extensions
{
    public static class ModelBuilderConstraintExtensions
    {
        public static void AddEntitiesConstraints(this ModelBuilder builder)
        {
            ...
            builder.Entity<CarModelDetails>().AddRangeConstraint(x => x.DoorCount, 1, 15);
            builder.Entity<CarModelDetails>().AddRangeConstraint(x => x.HorsePower, 1, 1000);
            builder.Entity<CarModelDetails>().AddRangeConstraint(x => x.AccelerationInSeconds, 0, 20m);
            builder.Entity<CarModelDetails>().AddRangeConstraint(x => x.MaxSpeedInKmPerHour, 1, 500);
            builder.Entity<CarModelDetails>().AddRangeConstraint(x => x.TrunkCapacityLiters, 0, 3000);
            builder.Entity<CarModelDetails>().AddRangeConstraint(x => x.TrunkCapacitySuitCases, 0, 100);
