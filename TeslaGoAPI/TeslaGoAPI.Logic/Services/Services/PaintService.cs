...
{
    public sealed class PaintService(IUnitOfWork unitOfWork, IAuthService authService)
       : GenericService<
           Paint,
           PaintRequestDto,
           PaintRequestDto,
           ...
           PaintQuery>(unitOfWork, authService), IPaintService
    {
        public sealed override async Task<Result<object>> DeleteAsync(int id)
        {
            ...
            var carsToDelete = await carsRepository.GetAllAsync(q => q.Where(x => x.PaintId == id));
